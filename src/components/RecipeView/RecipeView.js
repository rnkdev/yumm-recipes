...
class RecipeView extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.recipes !== this.props.recipes) {
      const nextQuery = nextProps.recipes.critera && nextProps.recipes.criteria.q ? nextProps.recipes.criteria.q : ''; 
          ...
          <GridList cellHeight={180} className={classes.gridList}>
            <GridListTile key="Subheader" cols={2} style={{ height: 'auto' }}>
              <Subheader component="div">
                Use the search bar above to search for recipe, press enter to begin search...
              </Subheader>
            </GridListTile> 
            {
              this.state.recipes ? this.state.recipes.map(recipe => (
                <GridListTile key={recipe.id} cols={1}>
                  <img src={getLImg(recipe.imageUrlsBySize['90'])} alt={recipe.recipeName}/>
                  <GridListTileBar
